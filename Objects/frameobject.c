        ...
        for (i = 0; i < len; i++) {
            ...
            switch (opcode) {
                ...
                case RERAISE:
                    ...
                    break;
                default: {
                    int delta = PyCompile_OpcodeStackEffect(opcode,
                                                            _Py_OPARG(code[i]));
                    while (delta < 0) {
                        next_stack = pop_value(next_stack);
                        delta++;
...
static void
init_frame(_PyInterpreterFrame *frame, PyFunctionObject *func,
           PyObject *locals) {
...
PyFrame_New(
    ...
    PyObject *builtins = _PyEval_BuiltinsFromGlobals(// borrowed ref
            tstate, globals
    );
    if (builtins == NULL) {
        return NULL;
...
_PyEval_BuiltinsFromGlobals(PyThreadState *tstate, PyObject *globals) {
    PyObject *builtins = PyDict_GetItemWithError(globals,
                                                 &_Py_ID(__builtins__));
    if (builtins) {
